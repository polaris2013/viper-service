...
//		System.out.println("----生成两两词之间的二叉图表，将结果保存在一个MultiTokenPairMap中" + this.getClass().getName());
//		System.out.println("\r\n !!--------创建图--------!!\r\n");
		double smooth = 0.1;
		int wordPairFreq = 0;
			...
			if (segGraph.isStartExist(key)) {
				...
				for (SegToken t1 : tokenList) {
					...
					for (SegToken t2 : nextTokens) {
						...
						weight = -Math.log(smooth * (1.0 + oneWordFreq) / (Utility.MAX_FREQUENCE + 0.0) + (1.0 - smooth)
...
//						System.out.println(new String(tokenPair.charArray) + " :  " + t1.index + "--->" + t2.index + "   weight:" + weight);
...
//		System.out.println("\r\n !!--------图创建完成--------!!");
//		System.out.println("图表现为一个map,记录了所有可能的组合，组合指的是前后两个相邻的词对。如‘我购买’可以拆作：    " + "\r\n   *我@购 :  1--->2 " + "\r\n   *我@购买 :  1--->3\r\n" + "两个组合\r\n");
	}
...
//		System.out.println("----用veterbi算法计算从起点到终点的最短路径,返回最优结果。本质是动态规划算法" + this.getClass().getName());
//		System.out.println("\r\n--开始计算最短路径--");
		int current;
		int nodeCount = getToCount();
//		System.out.println("--找到nodeCount：" + nodeCount);
		...
		for (current = 1; current <= nodeCount; current++) {
			...
			// -----------以下显示用
//			System.out.println("\r\n #####-获得相连的边，共" + edges.size() + "条。开始该edge的计算。请看显示：\r\n----------------");
			// -----------以上显示用
...
//				System.out.println("  --> " + new String(edge.charArray) + "    " + edge.from + "   " + edge.to + "  minweight： " + minWeight);
			}
...
//		System.out.println("----最短路径已经得到,即将返回最优结果。----------------" + this.getClass().getName());
